...
=========================\n");
  do{
	...
	exit(0);
      ...
      NNUM=nnum;
      printf("allocating memory for NEURONS[]\n");
    }
    // DEFINITION OF NEURONS
      ...
      int index=(int)(tmp);
      if(index!=tmp){
	printf("neuron id must be an integer!\n");
	exit(0);
      }
      if(index<0){
	...
	exit(0);
      ...
      // Layer 0 is just an input layer
      if(index == 1){
	printf("layer 1 should be just an input layer!\n");
      }
	...
	if(layer_id == 0){
	  NEURON[global_neuron_id].nw = 1;
	  NEURON[global_neuron_id].discriminant = LINEAR;
	  NEURON[global_neuron_id].w[0] = 1;
	  NEURON[global_neuron_id].activation = ID;
	}
      } else {
	...
	int neu=(int)(tmp);
    if(tmp!=neu){
      printf("TRAINING_POINT IN neuron index must be an integer!\n");
      exit(0);
    }
    if(neu<0){
      printf("TRAINING_POINT IN neuron index must be positive!\n");
      exit(0);
    }
    if(neu>(NNUM-1)){
    ...
    ret=fscanf(fp,"%lf",&tmp);
    printf("TRAINING_POINT IN %d %d %f [OK]\n",ind,neu,tmp);
    X[ind][neu]=tmp;
      }
    ...
    int neu=(int)(tmp);
    if(tmp!=neu){
      printf("TRAINING_POINT OUT neuron index must be an integer!\n");
      exit(0);
    }
    if(neu<0){
      printf("TRAINING_POINT OUT neuron index must be positive!\n");
      exit(0);
    }
    if(neu>(NNUM-1)){
   ...
   // verbosity = ON/OFF
    ...
    ret=fscanf(fp,"%lf",&tmp);
    double proto_var=tmp;
    if(proto_var <= 0){
     printf("Prototype variance must be positive!\n");
     exit(0);
    }
    PROTOTYPE_VARIANCE = proto_var;
    ret=fscanf(fp,"%lf",&tmp);
    double error_bound = tmp;
    if(error_bound <= 0){
      printf("Error bound must be positive!\n");
      exit(0);
    }
    ERROR_BOUND = error_bound;
    ret=fscanf(fp,"%lf",&tmp);
    int max_optimization_runs = (int)tmp;
    if(max_optimization_runs != tmp || max_optimization_runs <=0){
      printf("Maximum optimization runs must be a positive integer!\n");
      exit(0);
    }
    MAX_OPTIMIZATION_RUNS = max_optimization_runs;
    
    printf("OPTIMIZATION METHOD = BAYESIAN [OK]\n");
    VERBOSITY=verbosity;
  ...
  else if(strcmp(s,"NUMBER_OF_INPUT_POINTS")==0){
   ret=fscanf(fp,"%lf",&tmp);
   int num=(int)(tmp);
   if(tmp!=num){
    printf("NUMBER_OF_INPUT_POINTS must be an integer!\n");
    exit(0);
   }
   if(num<0){
    printf("NUMBER_OF_INPUT_POINTS must be positive!\n");
    exit(0);
   }
  ...
  // syntax: NETWORK_INPUT point_id neuron_id conn_id val
  else if(strcmp(s,"NETWORK_INPUT")==0){
   ret=fscanf(fp,"%lf",&tmp);
   int num=(int)(tmp);
   if(tmp!=num){
    printf("NETWORK_INPUT point index must be an integer!\n");
    exit(0);
   }
   if(num<0){
    printf("NETWORK_INPUT point index must be positive!\n");
    exit(0);
   }
   ...
   int neu=(int)(tmp);
   if(tmp!=neu){
    printf("NETWORK_INPUT neuron index must be an integer!\n");
    exit(0);
   }
   if(neu<0){
    printf("NETWORK_INPUT neuron index must be positive!\n");
    exit(0);
   }
   if(neu>(NETWORK.num_of_neurons[0]-1)){
   ...
   ret=fscanf(fp,"%lf",&tmp);
   val=tmp;
   printf("NETWORK INPUT POINT #%d %d = %g [OK]\n",num,neu,val);
   OUTPUT_X[num][neu]=val;
  }
  ...
  // syntax: SAVE_NEURAL_NETWORK
   ...
   ret=fscanf(fp,"%s",SAVE_NEURAL_NETWORK_FILENAME);
  ...
  // syntax: LOAD_NEURAL_NETWORK
   ...
   ret=fscanf(fp,"%s",LOAD_NEURAL_NETWORK_FILENAME);
   SAVE_NEURAL_NETWORK=ON;
  ...
  else if(strcmp(s,"INITIAL_WEIGHTS_RANDOMIZATION")==0){
   ret=fscanf(fp,"%s",s);
   if(strcmp(s,"ON")==0){
    INITIAL_WEIGHTS_RANDOMIZATION=ON;
    printf("INITIAL_WEIGHTS_RANDOMIZATION = ON [OK]\n");
   }
   else if(strcmp(s,"OFF")==0){
    INITIAL_WEIGHTS_RANDOMIZATION=OFF;
    printf("INITIAL_WEIGHTS_RANDOMIZATION = OFF [OK]\n");
   } else {
    printf("INITIAL_WEIGHTS_RANDOMIZATION can be only ON or OFF!\n");
    exit(0);
   }
  }
  // specify the error function for the training process
  // syntax: ERROR_TYPE L2/LINF
  else if(strcmp(s,"ERROR_TYPE")==0){
   ret=fscanf(fp,"%s",s);
   if(strcmp(s,"L2")==0){
    ERROR_TYPE=L2;
    printf("ERROR_TYPE = L2 [OK]\n");
   }
   else if(strcmp(s,"LINF")==0){
    ERROR_TYPE=LINF;
    printf("ERROR_TYPE = LINF [OK]\n");
   } else if(strcmp(s,"L2SQ")==0){
     ERROR_TYPE=L2SQ;
     printf("ERROR_TYPE = L2SQ [OK]\n");
   }else {
    printf("unknown specified error function!\n");
    exit(0);
   }
  }
